...
/**
 * The MicroService is an abstract class that any micro-service in the system must extend.
 * The abstract MicroService class is responsible to get and manipulate the singleton {@link MessageBus} instance.
 * <p>
 * Derived classes of MicroService never directly use the message-bus.
 * Instead, they have a set of internal protected wrapping methods (e.g., {@link #sendBroadcast(Broadcast)},
 * {@link #sendBroadcast(Broadcast)}, * etc.) they can use. When subscribing to message-types, the derived class also
 * supplies a {@link Callback} that is called when a message of the subscribed type was taken from the micro-service
 * message-queue (see {@link MessageBus#register(MicroService)} method). The abstract MicroService stores this callback
 * together with the type of the message is related to.
 * <p>
 */
public abstract class MicroService implements Runnable {
    ...
    /**
     * @param name the micro-service name (used mainly for debugging purposes - does not have to be unique)
     */
    ...
    * @param callback   The provided callback to associate with this Message type.
    ...
    /**
     * Subscribes to events of type {@code type} with the callback {@code callback}. This means two things:
     * 1. Subscribe to events in the singleton event-bus using the supplied
     * {@code type}
     * 2. Store the {@code callback} so that when events of type {@code type} are received it will be called.
     * <p>
     ...
     * @param callback The callback that is called when messages of type {@code type} are taken from this micro-service
     *                 message queue.
     */
     ...
     * 1. Subscribe to broadcast messages in the singleton event-bus using the supplied {@code type}
     * 2. Store the {@code callback} so that when broadcast messages of type {@code type} received it will be called.
     * <p>
     * For a received message {@code m} of type {@code type = m.getClass()} calling the callback {@code callback} means
     * running the method {@link Callback#call(java.lang.Object)} by calling {@code callback.call(m)}.
     * <p>
    ...
    /**
     * Sends the event {@code e} using the message-bus and receive a {@link Future<T>} object that may be resolved to
     * hold a result. This method is Non-Blocking since there may be events which do not require any response and resolving.
     * <p>
     * @param <T>       The type of the expected result of the request {@code e}
     * @param e         The event to send
     * @return  		{@link Future<T>} object that may be resolved later by a different micro-service processing
     *                  this event. null in case no micro-service has subscribed to {@code e.getClass()}.
     */
    ...
    /**
     * A Micro-Service calls this method in order to send the broadcast message {@code b} using the message-bus to all
     * the services subscribed to it.
     * <p>
    ...
    /**
     * Completes the received request {@code e} with the result {@code result} using the message-bus.
     * <p>
    ...
    /**
     * @return the name of the service - the service name is given to it in the construction time and is used mainly for
     * debugging purposes.
     */
    ...
    /**
     * The entry point of the micro-service. otherwise you will end up in an infinite loop.
     */
    @Override
